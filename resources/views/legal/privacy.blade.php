...
<head>
    ...
    <title>Política de Privacidade - {{ \App\Models\Setting::get('site_name', 'PayTech') }}</title>
        ...
        <article
            ...
            <header class="mb-12 border-b border-slate-100 pb-8">
                <div
                    ...
                    <i class="bi bi-shield-lock"></i> Privacidade & Segurança
                </div>
                <h1 class="text-4xl md:text-5xl font-black text-slate-900 tracking-tight italic uppercase mb-2">Política
                    de Privacidade</h1>
            ...
            <div class="legal-content max-w-none">
                @php
                    $content = \App\Models\Setting::get('legal_privacy_content');
                @endphp
                ...
                @if (empty($content) || trim(strip_tags($content)) == '')
                    <div class="bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl p-12 text-center">
                        <i class="bi bi-shield-lock text-4xl text-slate-300 mb-4 block"></i>
                        <p class="text-slate-400 font-medium italic">A Política de Privacidade ainda não foi configurada
