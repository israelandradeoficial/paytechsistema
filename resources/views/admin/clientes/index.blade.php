...
@section('content')
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($cliente->status === 'ativo')
                                                    <span
                                                        class="badge bg-success-subtle text-success border border-success-subtle">Ativo</span>
                                                @else
                                                    <span
                                                        class="badge bg-danger-subtle text-danger border border-danger-subtle">Inativo</span>
                                                @endif
                                            </td>
                                            <td>
                                                @if ($cliente->data_validade)
                                                    @if ($cliente->data_validade->lt(today()))
                                                        <span
                                                            class="badge bg-warning-subtle text-warning border border-warning-subtle"><i
                                                                class="bi bi-clock-history me-1"></i>{{ $cliente->data_validade->format('d/m/Y') }}</span>
                                                    @else
                                                        <span
                                                            class="text-muted small">{{ $cliente->data_validade->format('d/m/Y') }}</span>
                                                    @endif
                                                @else
                                            ...
                                            <td class="text-end pe-4">
                                                <div class="btn-group">
                                                    <a href="{{ route('simulator.index') }}" target="_blank"
                                                    ...
                                                    @can('manage_rates')
                                                        <button type="button" class="btn btn-outline-primary btn-sm border-0"
                                                            data-bs-toggle="modal" data-bs-target="#modalTaxas"
                                                    ...
                                                    @can('manage_clients')
                                                        <button type="button"
                                                            class="btn btn-outline-danger btn-sm border-0 btn-delete-cliente"
                                                            data-id="{{ $cliente->id }}" data-nome="{{ $cliente->nome }}">
    ...
    <script>
        // Clientes already has some AJAX implementation, but let's unify it with our global setup
        document.addEventListener('DOMContentLoaded', function() {
            // Handle Novo Cliente
            handleAjaxForm('formNovoCliente', 'modalNovoCliente');

            // Handle Editar Cliente
            handleAjaxForm('formEditarCliente', 'modalEditarCliente');
        });
